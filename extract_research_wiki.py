...
# Check RESEARCH for Wikipedia links
research_dir = '/mnt/d/📂ORGANIZATION_META/WORKSPACE/BOOKMARKS/RESEARCH'
wiki_links = []

for fname in os.listdir(research_dir):
    if fname.endswith('.md') and fname != '_INDEX.md':
        with open(f'{research_dir}/{fname}', 'r') as f:
            content = f.read()
...
print(f'Total Wikipedia links in RESEARCH: {len(wiki_links)}')
...
# Save
import json
with open('/mnt/d/📂ORGANIZATION_META/WORKSPACE/BOOKMARKS/research_wiki.json', 'w') as f:
    json.dump({
